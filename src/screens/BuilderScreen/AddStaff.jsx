...
import {  add_staff_builder, my_properties_builder, my_properties_for_owner, staff_responsibility_master, staff_role_master } from "../../api/auth";
                  ...
                  <div class="submit-section">
                    ...
                    <div class="form-group col-md-6">
                      ...
                      <div className="form-control" style={{ height: 'auto' }}>
                        {resposibilityMaster?.map((item) => (
                          <label key={item.name} style={{ display: 'block' }}>
                            <input
                              type="checkbox"
                              value={item.name}
                              checked={Res.includes(item.name)}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  handleChange([...Res, e.target.value]); // Add item to selected list
                                } else {
                                  handleChange(Res.filter((res) => res !== e.target.value)); // Remove item from selected list
                                }
                              }}
                            />
                            {item.name}
