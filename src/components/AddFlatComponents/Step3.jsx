...
const Step3 = ({
  ...
  useEffect(() => {
    setProperty(globalData?.user?.property_available?.toString())
    setCurrency(globalData?.user?.currency)
    setSellingPrice(globalData?.user?.selling_price.toString())
    setMonthly(globalData?.user?.monthly_maintenance)
    setRent(globalData?.user?.expected_rent.toString())
    setDeposit(globalData?.user?.expected_deposit)
    setAvailble(globalData?.user?.available_from)
    setFurnishing(globalData?.user?.furnishing)
  },[])
